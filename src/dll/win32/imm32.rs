...
impl IMM32 {
    ...
    // 역할: 현재 변환 상태를 가져옴
    pub fn imm_get_conversion_status(uc: &mut Unicorn<Win32Context>) -> Option<ApiHookResult> {
    ...
    // 역할: 현재 변환 상태를 설정
    pub fn imm_set_conversion_status(uc: &mut Unicorn<Win32Context>) -> Option<ApiHookResult> {
        let himc = uc.read_arg(0);
