...
impl DllKERNEL32 {
    ...
    pub fn wait_for_single_object(uc: &mut Unicorn<Win32Context>) -> Option<ApiHookResult> {
        ...
        let in_thread = uc.get_data().current_thread_idx.load(Ordering::SeqCst) > 0;

        let timeout_ms: u64 = if in_thread {
            dw_milliseconds.min(500) as u64
        } else if dw_milliseconds == 0xFFFFFFFF {
            30_000
        } else {
            dw_milliseconds as u64
        };
        ...
        if let Some(entry) = wsa_entry {
            ...
            // 소켓 readable 대기 (Mutex를 잡은 채로 block_on)
            let sockets = ctx.tcp_sockets.lock().unwrap();
            if let Some(s) = sockets.get(&sock) {
                if let Some(stream) = &s.stream {
                    let result = get_tokio_runtime().block_on(async {
                        tokio::time::timeout(
                    ...
                    match result {
                        Ok(Ok(_)) => {
                            ctx.wsa_event_map
                                ...
                                .and_modify(|e| e.pending |= 0x01); // FD_READ
                            crate::emu_log!(
                                "[KERNEL32] WaitForSingleObject(WSA event={:#x}) -> WAIT_OBJECT_0 (readable)",
                                h_handle
                            );
                            return Some(ApiHookResult::callee(2, Some(0))); // WAIT_OBJECT_0
                        }
                        _ => {
                            ...
                            return Some(ApiHookResult::callee(2, Some(0x102))); // WAIT_TIMEOUT
                        }
                    }
                }
                drop(sockets);
            }
            // 소켓이 없거나 스트림 없음 → timeout
            crate::emu_log!(
                "[KERNEL32] WaitForSingleObject(WSA event={:#x}) -> WAIT_TIMEOUT (no stream)",
                h_handle
            );
            return Some(ApiHookResult::callee(2, Some(0x102))); // WAIT_TIMEOUT
        }

        // 일반 동기화 객체 → 스레드 스케줄링 후 반환
        crate::emu_log!(
            "[KERNEL32] WaitForSingleObject({:#x}, {}) -> WAIT_OBJECT_0",
            h_handle,
            dw_milliseconds
        );
        if !in_thread {
            uc.apply_stack_cleanup(StackCleanup::Callee(2));
            DllKERNEL32::schedule_threads(uc);
            crate::emu_log!(
                "[KERNEL32] Returning from WaitForSingleObject({:#x}) toward guest",
                h_handle
            );
            return Some(ApiHookResult::callee(0, Some(0))); // WAIT_OBJECT_0 (cleanup done)
        }

        // 배경 스레드에서 대기 시: 핸들이 시그널 상태가 아니면 retry()로 양보
        // (현재는 항상 성공으로 리턴하던 것을 개선 가능하지만, 일단 무조건 성공 대신 retry 테스트)
        // FIXME: 실제 오브젝트 상태 확인 로직 필요
        // return Some(ApiHookResult::retry());
        ...
        Some(ApiHookResult::callee(2, Some(0))) // WAIT_OBJECT_0
    }
    ...
    pub fn wait_for_multiple_objects(uc: &mut Unicorn<Win32Context>) -> Option<ApiHookResult> {
                        ...
                        match result {
                            Ok(Ok(_)) => {
                                ctx.wsa_event_map
                                    ...
                                    .and_modify(|e| e.pending |= 0x01);
                                crate::emu_log!(
                                    "[KERNEL32] WaitForMultipleObjects: WSA event={:#x} readable -> WAIT_OBJECT_0+{}",
                                    handle,
                                    i
                                );
                                return Some(ApiHookResult::callee(4, Some(i as i32)));
                            ...
                            _ => {
                                crate::emu_log!(
                                    "[KERNEL32] WaitForMultipleObjects: WSA event={:#x} timeout -> WAIT_TIMEOUT",
                                    handle
                                );
                                return Some(ApiHookResult::callee(4, Some(0x102))); // WAIT_TIMEOUT
                            }
                        }
                    }
                    drop(sockets);
        ...
        let in_thread = uc.get_data().current_thread_idx.load(Ordering::SeqCst) > 0;
        if !in_thread {
            uc.apply_stack_cleanup(StackCleanup::Callee(4));
            DllKERNEL32::schedule_threads(uc);
            crate::emu_log!(
                "[KERNEL32] Returning from WaitForMultipleObjects({}) toward guest",
                n_count
            );
            return Some(ApiHookResult::callee(0, Some(0))); // WAIT_OBJECT_0
        }
        ...
        Some(ApiHookResult::callee(4, Some(0))) // WAIT_OBJECT_0
    }
    ...
    pub(crate) fn schedule_threads(uc: &mut Unicorn<Win32Context>) {
        ...
        for i in 0..count {
            ...
            let alive = !terminate_requested && new_eip != EXIT_ADDRESS as u32;
            {
                let ctx = uc.get_data();
                let mut threads = ctx.threads.lock().unwrap();
        ...
        // 메인 스레드로 복귀
        uc.get_data().current_thread_idx.store(0, Ordering::SeqCst);
    ...
    pub fn create_thread(uc: &mut Unicorn<Win32Context>) -> Option<ApiHookResult> {
        ...
        let _dw_creation_flags = uc.read_arg(4);
        ...
        crate::emu_log!(
            "[KERNEL32] CreateThread(entry={:#x}, param={:#x}) -> handle={:#x}, id={:#x}",
            lp_start_address,
            lp_parameter,
            handle,
            thread_id
    ...
    pub fn handle(uc: &mut Unicorn<Win32Context>, func_name: &str) -> Option<ApiHookResult> {
        match func_name {
            ...
            "CreateThread" => DllKERNEL32::create_thread(uc),
            _ => {
